...
func dataSourceGithubReleaseRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[INFO] in github_release")

	var err = client.Query(context.Background(), &query, variables)
	if err != nil {
		fmt.Errorf("failed to validate returned client without error: %s", err.Error())
	}
